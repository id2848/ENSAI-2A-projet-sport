...
class Activite:
    ...
    def __repr__(self):
        return (
            f"Activite(id_activite={self.id_activite}, "
    ...
    def valider_sport(sport: str) -> str:
        ...
        sport = sport.lower()
        if sport not in ["course", "natation", "vélo", "randonnée", "autre"]:
            raise ValueError(
                "Sport invalide. Il doit être 'course', 'natation', 'vélo', 'randonnée' ou 'autre'"
            )
        return sport
    ...
    def valider_date_activite(date_activite: date | str) -> date:
        ...
        try:
            ...
            raise ValueError(
                "date_activite doit être un objet 'date' ou un str au format YYYY-MM-DD"
            )
