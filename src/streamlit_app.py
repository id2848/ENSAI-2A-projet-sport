...
def normalize_activity(activite_raw: dict) -> dict:
    ...
    a['id_utilisateur'] = extract_activity_field(activite_raw, 'id_utilisateur', 'id_auteur', default=None)
...
def gpx_analyse_et_creation():
    ...
    if 'analyse_data' in st.session_state:
        ...
        sport = st.selectbox("Confirmer le sport", ["course", "vélo", "randonnée", "natation", "autre"])
...
def afficher_activites_personnelles():
    ...
    sport = st.selectbox("Filtrer par sport (optionnel)", ["", "course", "vélo", "randonnée", "natation", "autre"])
    ...
    if st.button("Actualiser mes activités"):
        ...
        # 🔥 Nouvel endpoint utilisé ici
        resp = requests.get(
            f"{API_ACTIVITES_FILTRES}/{user_id}",
            params=params,
            auth=auth_tuple()
        )
...
def afficher_statistiques():
    ...
    if 'total_stats' in st.session_state:
        ...
        # API renvoies: nombre_activites_total, distance_totale, duree_totale
